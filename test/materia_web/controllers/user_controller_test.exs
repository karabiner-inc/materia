defmodule MateriaWeb.UserControllerTest do
  ...
  describe "admin_user" do
    test "admin user ligin cycle", %{conn: conn} do
      ...
      assert json_response(role_all_conn, 200) |> Enum.chunk_every(2) |> List.first() == [
               %{"id" => 1, "method" => "ANY", "request_path" => "/api/ops/users", "role" => "anybody"},
               %{"id" => 2, "request_path" => "/api/ops/grants", "role" => "admin", "method" => "ANY"}
             ]
  ...
  describe "tmp_user life cycle" do
    test "tmp_user registration flow", %{conn: conn} do
      # 仮登録する
      tmp_conn1 =
        post(conn, user_path(conn, :registration_tmp_user), %{
          "email" => "tmp_user_test001@example.com",
          "role" => "customer"
        })
      ...
      # 本登録する(tokenがなければエラー)
      reg_conn1 =
        post(conn, user_path(conn, :registration_user), %{
          "name" => "tmp_user_test001@example.com",
          "password" => "password"
        })
      ...
      reg_conn1 =
        post(conn, user_path(conn, :registration_user), %{"name" => "tmp_user_test001 username", "password" => ""})
      ...
      reg_conn1 =
        post(conn, user_path(conn, :registration_user), %{
          "name" => "tmp_user_test001 username",
          "password" => "password"
        })
    ...
    test "tmp user registration invalid status", %{conn: conn} do
      ...
      tmp_conn1 =
        post(conn, user_path(conn, :registration_tmp_user), %{
          "email" => "tmp_user_test002@example.com",
          "role" => "customer"
        })
      ...
      # ユーザーを凍結
      conn_update1 =
        put(conn_auth, user_path(conn_auth, :update, resp1["user"]["id"]), %{"status" => User.status().frozen})
      ...
      # ユーザーを失効
      conn_update2 =
        put(conn_auth, user_path(conn_auth, :update, resp1["user"]["id"]), %{"status" => User.status().expired})
  ...
  describe "password reset flow" do
    test "password reset cycle", %{conn: conn} do
      ...
      # ユーザー登録 :create
      conn_create1 =
        post(conn_auth, user_path(conn_auth, :create), %{
          "name" => "pw_reset0001 user",
          "email" => "pw_reset0001@example.com",
          "role" => "customer",
          "password" => "password"
        })
      ...
      # ログインできる
      token_conn =
        post(conn, authenticator_path(conn, :sign_in), %{
          "email" => "pw_reset0001@example.com",
          "password" => "password"
        })
      ...
      assert resp_err ==
               "[{\"title\":\"401 Unauthorized\",\"status\":401,\"id\":\"UNAUTHORIZED\",\"detail\":\"email or password is invalid\"}]"
      ...
      # ユーザーを凍結
      conn_update1 =
        put(conn_auth, user_path(conn_auth, :update, resp_create1["id"]), %{"status" => User.status().frozen})
      ...
      # ユーザーを失効
      conn_update2 =
        put(conn_auth, user_path(conn_auth, :update, resp_create1["id"]), %{"status" => User.status().expired})
