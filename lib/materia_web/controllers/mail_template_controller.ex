defmodule MateriaWeb.MailTemplateController do
  ...
  alias Materia.Mails
  alias Materia.Mails.MailTemplates
  ...
  def create(conn, %{"mail_template" => mail_template_params}) do
    with {:ok, %mail_template{} = mail_template} <- Mails.create_mail_template(mail_template_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "mail_template" => mail_template_params}) do
    ...
    with {:ok, %mail_template{} = mail_template} <- Mails.update_mail_template(mail_template, mail_template_params) do
      render(conn, "show.json", mail_template: mail_template)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    mail_template = Mails.get_mail_template!(id)
    with {:ok, %mail_template{}} <- Mails.delete_mail_template(mail_template) do
